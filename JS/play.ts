...
let numberBeforeOperator: string = "";
let numbersInCalculator: string[] = [];
let temporaryResult: number | null = null;
let equalsPressed = false;

//Operator functions
function add(firstOperand: string, secondOperand: string) {
  temporaryResult =
...
function subtract(firstOperand: string, secondOperand: string) {
  temporaryResult =
    Math.round((Number(firstOperand) - Number(secondOperand)) * 100) / 100;
  displayCalculation.textContent = temporaryResult.toString();
  numbersInCalculator.splice(0, 2, temporaryResult.toString());
...
function multiply(firstOperand: string, secondOperand: string) {
  temporaryResult =
    Math.round(Number(firstOperand) * Number(secondOperand) * 100) / 100;
  displayCalculation.textContent = temporaryResult.toString();
  numbersInCalculator.splice(0, 2, temporaryResult.toString());
...
function divide(firstOperand: string, secondOperand: string) {
  if (secondOperand === "0") {
...
for (let i = 0; i < 11; i++) {
  const operandButtons: HTMLButtonElement[] = [];
